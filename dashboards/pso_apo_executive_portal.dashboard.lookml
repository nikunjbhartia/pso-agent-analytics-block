- dashboard: pso_apo_executive_portal
  ...
  - name: resilience_rate_pct_card
    ...
    note_state: collapsed
    note_display: hover
    note_text: "<div style=\"text-align: left;\">What: Reliability SLA metric measuring system stability. <br><br>How: Ratio of SUCCESS outcomes vs total executions (COUNTIF(status = SUCCESS) / COUNT(1)). <br><br>Why it matters: Asserts production stability and CI/CD readiness. <br><br>Drill: Click tile to inspect failing tool tracebacks.</div>"
    explore: agent_events
    model: bigquery_agent_analytics_model
  ...
  - name: graph_trace_dag_and_sla_gate_table
    ...
    note_state: collapsed
    note_display: hover
    note_text: "<div style=\"text-align: left;\">What: Combined ROI leaderboard and CI/CD SLA Gate status table. <br><br>How: CWPM Hours = tool calls * 1.5 * 1.2, SLA Gate = PASS if error rate <= 5 percent. <br><br>Why it matters: Unifies financial ROI with technical deployment readiness. <br><br>Drill: Click any agent to drill into DAG lineage.</div>"
    explore: agent_events
    model: bigquery_agent_analytics_model
  ...
  - name: resilience_sla_by_practice_area_column
    ...
    note_state: collapsed
    note_display: hover
    note_text: "<div style=\"text-align: left;\">What: Overlay of event volume and reliability SLA across practice areas. <br><br>How: Columns = total_events, Line = self-healing resilience rate (SUCCESS / Total). <br><br>Why it matters: Ensures high-traffic practices maintain >=95% SLA. <br><br>Drill: Click practice column to inspect reliability.</div>"
    explore: agent_events
    model: bigquery_agent_analytics_model
    dimensions: [agent_events.practice_area]
  ...
  - name: bqml_roi_30day_forecast_chart
    ...
    note_state: collapsed
    note_display: hover
    note_text: "<div style=\"text-align: left;\">What: 30-day predictive forecast of automation hours saved and consulting dollar value. <br><br>How: Historical actuals use total_sessions * 3.5h ($350/hr PSO rate); future 30-day predictions use linear growth trend projection with 90%/110% confidence bounds. <br><br>Why it matters: Anchors quarterly capacity planning and financial projections. <br><br>Drill: Filter by Practice Area or Date Range.</div>"
    explore: agent_events
    model: bigquery_agent_analytics_model
  ...
  - name: session_trace_dag_lineage_graph
    ...
    note_state: collapsed
    note_display: hover
    note_text: "<div style=\"text-align: left;\">What: Hierarchical DAG execution flow across session IDs and trace hops. <br><br>How: Extracts from_agent -> to_target delegation hops from agent_events where event_type is AGENT_TRANSFER, A2A_INTERACTION, or TOOL_COMPLETED. <br><br>Why it matters: Maps multi-agent orchestration paths and latency bottlenecks. <br><br>Drill: Filter by Session ID or Trace ID to inspect specific DAGs.</div>"
    explore: agent_events
    model: bigquery_agent_analytics_model
