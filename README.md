...
*   **Where AI Recommendations Come From (3-Tier Evaluation Engine)**: LookML view `v_agent_evaluation` uses a 3-tier fallback (`COALESCE`) to populate actionable prompt recommendations: **(1) Python UDFs (`bqaa_score_*`)** compute zero-cost numeric quality scores without an LLM; **(2) Scheduled AI Judge (`AI.GENERATE_TEXT` with `gemini-2.5-flash`)** runs every 15 minutes to synthesize structured JSON prompt engineering fixes (`{"priority": "P1", "prompt_fix": "..."}`); and **(3) Remote Function (`agent_analytics('evaluate')`)** allows analysts to run live LLM-as-a-Judge evaluations on demand via SQL (see **[METRICS.md Section 4](METRICS.md#4-deep-dive-ai-recommendation-provenance--which-engine-generates-what)**).
